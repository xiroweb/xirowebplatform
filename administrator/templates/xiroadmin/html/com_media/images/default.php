...
JFactory::getDocument()->addScriptDeclaration('
	 jQuery(document).ready(function($) {
	 	var countoption =   $("#folderlist option").length;
	 	$("#folderlist").attr("size", countoption);
					...
					<form action="index.php?option=com_media&amp;asset=<?php echo $asset; ?>&amp;author=<?php echo $author; ?>" class="form-horizontal" id="imageForm" method="post" enctype="multipart/form-data">
						...
						<div class="well">
							<div class="row-fluid">
								...
								<div class="span12 control-group">
									<button class="btn" type="button" id="upbutton" title="<?php echo JText::_('COM_MEDIA_DIRECTORY_UP'); ?>"><span class="icon-box-remove">&nbsp;</span> <?php echo JText::_('COM_MEDIA_UP'); ?></button>
									<div class="pull-right">
										<button class="btn btn-success button-save-selected" type="button" <?php if (!empty($onClick)) :
											// This is for Mootools compatibility ?>onclick="<?php echo $onClick; ?>"<?php endif; ?> data-dismiss="modal"><span class="icon-enter">&nbsp;</span> <?php echo JText::_('COM_MEDIA_INSERT'); ?></button>
										<button class="btn button-cancel" type="button" onclick="window.parent.jQuery('.modal.in').modal('hide');<?php if (!empty($onClick)) :
											// This is for Mootools compatibility ?>parent.jModalClose();<?php endif ?>" data-dismiss="modal"><?php echo JText::_('JCANCEL'); ?> <span class="icon-delete">&nbsp;</span></button>
									</div>
								</div>
							</div>
						</div>
